...
const getFirstWord = (str = "") => str.trim().split(" ")[0] || "";

const formatRole = (role = "") => {
  // Reemplaza _ por espacio y pone en mayúscula la primera letra
  const withSpaces = role.replace(/_/g, " ").toLocaleLowerCase("es");
...
const Inicio = () => {
  ...
  if (!usuario) {
    // useAuth redirige si no hay sesión
    return null;
  }

  /* ------------------- datos de encabezado ------------------- */
  const grupoPrincipalObj =
    grupos.length > 0 ? grupos[0] : { nombre: "Sin grupo", rol: usuario.rol };

  const grupoPrincipal = grupoPrincipalObj.nombre;
  const rolPrincipal = grupoPrincipalObj.rol || usuario.rol;

  const firstName = getFirstWord(usuario.nombre || "");
  const firstSurname = getFirstWord(usuario.apellido || usuario.apellidos || "");

const companyDisplay = /^personal-/i.test(grupoPrincipal) ? "N/A" : grupoPrincipal;

  const roleDisplay = formatRole(rolPrincipal);
    ...
    return (
      <div
        className={`
          ...
          ${isLocked ? "opacity-60 cursor-not-allowed" : "hover:shadow-xl hover:-translate-y-1 cursor-pointer"}
        `}
      >
        {/* Gradiente superior */}
        ...
        <div className="p-6">
          ...
          {isLocked ? (
            <div className="text-red-500 text-sm font-medium">Sin permisos de acceso</div>
          ) : (
            <Link
              to={module.path}
              className={`
  ...
  /* ------------------- render ------------------- */
  return (
    <div id="root" className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div className="mb-6 lg:mb-0">
              <h1 className="text-4xl font-bold text-gray-800 mb-2">
                Bienvenido, {firstName} {firstSurname} 👋
              </h1>
              <p className="text-gray-600 text-lg">Sistema de Gestión de Inventario</p>
            </div>
            ...
            <div className="bg-gradient-to-r from-teal-50 to-teal-100 rounded-lg p-6">
              ...
              <div className="space-y-1">
                <p className="text-sm text-gray-700">
                  <span className="font-medium">Empresa:</span> {companyDisplay}
                </p>
                <p className="text-sm text-gray-700">
                  <span className="font-medium">Rol:</span> {roleDisplay}
                </p>
              </div>
            </div>
          </div>
