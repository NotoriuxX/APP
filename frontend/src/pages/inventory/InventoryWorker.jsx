...
export default function InventoryWorker() {
  ...
  const [userFormData, setUserFormData] = useState({
    ...
    permisos_especiales: []  // Permisos específicos del usuario
  });
        ...
        if (numTrabajadores === 0) {
          ...
          if (window.confirm(confirmMessage)) {
            ...
            for (const trabajador of trabajadoresConOcupacion) {
              try {
                console.log(`📝 Quitando ocupación "${itemName}" a ${trabajador.nombres} ${trabajador.apellidos} (ID: ${trabajador.id})`);
                ...
                if (updateResponse.ok) {
                  ...
                  console.log(`✅ Ocupación quitada a ${trabajador.nombres} ${trabajador.apellidos}`);
                } else {
                  erroresActualizacion++;
                  console.log(`❌ Error al actualizar ${trabajador.nombres} ${trabajador.apellidos}:`, updateResponse.status);
  ...
  const handleDepartamentoChange = (value) => {
    ...
    const filteredDepts = departamentos.filter(dept => 
      dept.nombre && dept.nombre.toLowerCase().includes(trimmedValue.toLowerCase())
    );
  ...
  const handleOcupacionChange = (value) => {
    ...
    const filteredOcupaciones = ocupaciones.filter(ocupacion => 
      ocupacion && ocupacion.toLowerCase().includes(trimmedValue.toLowerCase())
    );
  ...
  useEffect(() => {
    ...
    if (usuario && !authLoading) {
      ...
      fetchAvailableRoles(); // Cargar roles disponibles
    }
  ...
  // Función para obtener módulos disponibles
  const fetchAvailableModules = async () => {
    try {
      const response = await fetch(`${API_URL}/modules/available`);
      if (!response.ok) {
  ...
  const fetchAvailableRoles = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/roles/available`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        ...
        // Si no hay endpoint, usar roles por defecto
        setAvailableRoles([
          { id: 'trabajador', nombre: 'trabajador', descripcion: 'Trabajador - Acceso básico a funciones asignadas' },
          { id: 'tecnico', nombre: 'tecnico', descripcion: 'Técnico - Gestión técnica y mantenimiento' },
          { id: 'supervisor', nombre: 'supervisor', descripcion: 'Supervisor - Supervisión de departamentos y reportes' },
          { id: 'coordinador', nombre: 'coordinador', descripcion: 'Coordinador - Coordinación de actividades' },
          { id: 'encargado', nombre: 'encargado', descripcion: 'Encargado - Responsable de una sección específica' },
          { id: 'jefe_departamento', nombre: 'jefe_departamento', descripcion: 'Jefe de departamento - Liderazgo departamental' },
          { id: 'gerente', nombre: 'gerente', descripcion: 'Gerente - Supervisión completa de su área' },
          { id: 'administrador', nombre: 'administrador', descripcion: 'Administrador - Control total del sistema' }
        ]);
      }
    } catch (error) {
      ...
      setAvailableRoles([
        { id: 'trabajador', nombre: 'trabajador', descripcion: 'Trabajador' },
        { id: 'tecnico', nombre: 'tecnico', descripcion: 'Técnico' },
        { id: 'supervisor', nombre: 'supervisor', descripcion: 'Supervisor' },
        { id: 'administrador', nombre: 'administrador', descripcion: 'Administrador' }
      ]);
    }
  };
  ...
  // Función para obtener permisos predeterminados según el rol
  const getDefaultPermissionsByRole = (rolNombre) => {
    const permissionsMap = {
      'encargado_impresiones': [
        'trabajador_leer',
        'fotocopia_leer', 'fotocopia_escribir', 'fotocopia_editar', 'fotocopia_eliminar'
      ],
      'trabajador': [
        'trabajador_leer',
        'fotocopia_leer', 'fotocopia_escribir'
      ],
      'operario': [
        'trabajador_leer',
        'inventario_leer', 'inventario_escribir', 'inventario_editar',
        'fotocopia_leer', 'fotocopia_escribir', 'fotocopia_editar'
      ],
      'tecnico': [
        'trabajador_leer', 'trabajador_escribir', 'trabajador_editar',
        'inventario_leer', 'inventario_escribir', 'inventario_editar',
        'fotocopia_leer', 'fotocopia_escribir', 'fotocopia_editar'
      ],
      'supervisor': [
      ...
      'jefe_departamento': [
        'trabajador_leer', 'trabajador_escribir', 'trabajador_editar', 'trabajador_eliminar',
        'inventario_leer', 'inventario_escribir', 'inventario_editar',
        'fotocopia_leer', 'fotocopia_escribir', 'fotocopia_editar', 'fotocopia_eliminar'
      ],
      'gerente': [
        'trabajador_leer', 'trabajador_escribir', 'trabajador_editar', 'trabajador_eliminar',
        'inventario_leer', 'inventario_escribir', 'inventario_editar', 'inventario_eliminar',
        'fotocopia_leer', 'fotocopia_escribir', 'fotocopia_editar', 'fotocopia_eliminar'
      ],
      'administrador': [
        'trabajador_leer', 'trabajador_escribir', 'trabajador_editar', 'trabajador_eliminar',
    ...
    return permissionsMap[rolNombre] || permissionsMap['trabajador'];
  };
  ...
  const handleUserInputChange = (e) => {
    ...
    // Si cambió el rol, auto-seleccionar permisos según el cargo
    if (name === 'rol_grupo') {
      const roleName = value;
      let permisosSugeridos = getDefaultPermissionsByRole(roleName);
      
      console.log(`Cambiando a rol ${roleName}, permisos sugeridos:`, permisosSugeridos);
      
      // Si es encargado_impresiones, asegurar que solo tenga los permisos permitidos
      if (roleName === 'encargado_impresiones') {
        const permisosPermitidos = ['trabajador_leer', 'fotocopia_leer', 'fotocopia_escribir', 'fotocopia_editar', 'fotocopia_eliminar'];
        permisosSugeridos = permisosSugeridos.filter(p => permisosPermitidos.includes(p));
      }
      
      setUserFormData(prev => ({
        ...prev,
        [name]: value,
        permisos_especiales: permisosSugeridos // Auto-seleccionar permisos según el cargo
      }));
  ...
  const openEditForm = (trabajador) => {
    ...
    const formDataToSet = {
      ...
      ocupacion: trabajador.cargo || '', // Mapear cargo a ocupacion para el formulario
      email: trabajador.email || '',
      ropera: trabajador.rut || '',
      departamento: (trabajador.departamento_nombre && trabajador.departamento_nombre !== 'Sin departamento') ? trabajador.departamento_nombre : '',
    ...
    setOriginalWorkerData({
      departamento: (trabajador.departamento_nombre && trabajador.departamento_nombre !== 'Sin departamento') ? trabajador.departamento_nombre : '',
      ocupacion: trabajador.cargo || '' // Mapear cargo a ocupacion
    });
  ...
  const handleSubmit = async (e) => {
    ...
    const continueWithSave = async () => {
      ...
      try {
        ...
        const formDataWithGroup = {
          ...
          cargo: formData.ocupacion.trim(), // Mapear ocupacion a cargo
    ...
    try {
      ...
      if (editingId && originalWorkerData) {
        console.log('🔍 Verificando cambios en departamento/ocupación');
        console.log('Original departamento:', originalWorkerData.departamento);
        console.log('Nuevo departamento:', formData.departamento.trim());
        console.log('Original ocupación:', originalWorkerData.ocupacion);
        console.log('Nueva ocupación:', formData.ocupacion.trim());
        
        let needsConfirmation = false;
        
        // Solo verificar si se está creando un departamento/ocupación NUEVO
        // Si el departamento/ocupación ya existe, actualizar sin preguntar
        
        // Verificar si se está creando un nuevo departamento
        if (formData.departamento.trim() !== '' && 
            originalWorkerData.departamento !== formData.departamento.trim()) {
          
          const departamentoExiste = departamentos.some(dept => 
            dept.nombre && dept.nombre.toLowerCase().trim() === formData.departamento.toLowerCase().trim()
          );
          
          if (!departamentoExiste) {
            // Solo preguntar si es un departamento completamente nuevo
        ...
        // Verificar si se está creando una nueva ocupación
        if (!needsConfirmation && 
          ...
          const ocupacionExiste = ocupaciones.some(ocup => 
            ocup && ocup.toLowerCase().trim() === formData.ocupacion.toLowerCase().trim()
          );
          
          if (!ocupacionExiste) {
            // Solo preguntar si es una ocupación completamente nueva
            showGlobalChangeConfirmation(
              'ocupacion', 
      ...
      console.log('✅ No hay confirmaciones pendientes, continuando con guardado');
  ...
  const getSelectedWorkerNames = () => {
    return allTrabajadores
      ...
      .map(worker => `${worker.nombres} ${worker.apellidos}`);
  };
  ...
  const getFilteredTrabajadores = () => {
    return allTrabajadores.filter(trabajador => {
      ...
      if (filters.search) {
        ...
        const matchesSearch = 
          trabajador.nombres?.toLowerCase().includes(searchTerm) ||
          trabajador.apellidos?.toLowerCase().includes(searchTerm) ||
  ...
  const getSortedTrabajadores = () => {
    ...
    if (sortConfig.key) {
      sortedTrabajadores.sort((a, b) => {
        ...
        if (sortConfig.key === 'nombre' || sortConfig.key === 'nombres') {
          aValue = `${a.nombres} ${a.apellidos}`;
          bValue = `${b.nombres} ${b.apellidos}`;
        } else if (sortConfig.key === 'departamento' || sortConfig.key === 'departamento_nombre') {
  ...
  const openUserModal = (trabajador) => {
    ...
    const rolInicial = 'trabajador';
    const permisosIniciales = getDefaultPermissionsByRole(rolInicial);
    
    setUserFormData({
      email: trabajador.email || '',
      password: '',
      confirmPassword: '',
      rol_grupo: rolInicial,
      permisos_especiales: permisosIniciales
    });
  ...
  // Función para manejar selección de módulo completo
  const toggleModuleSelection = (moduleKey, subPermissions) => {
    const allSelected = subPermissions.every(perm => 
      userFormData.permisos_especiales.includes(perm.codigo)
    );
    ...
    if (allSelected) {
      ...
      setUserFormData(prev => ({
        ...prev,
        permisos_especiales: prev.permisos_especiales.filter(p => 
          !subPermissions.some(sub => sub.codigo === p)
        )
      }));
    } else {
      // Seleccionar todos los permisos del módulo
      const newPermissions = subPermissions.map(perm => perm.codigo);
      setUserFormData(prev => ({
        ...prev,
        permisos_especiales: [...new Set([...prev.permisos_especiales, ...newPermissions])]
      }));
    }
  };

  // Función para verificar si un módulo está completamente seleccionado
  const isModuleFullySelected = (subPermissions) => {
    return subPermissions.every(perm => 
      userFormData.permisos_especiales.includes(perm.codigo)
    );
  };

  // Función para verificar si un módulo está parcialmente seleccionado
  const isModulePartiallySelected = (subPermissions) => {
    return subPermissions.some(perm => 
      userFormData.permisos_especiales.includes(perm.codigo)
    ) && !isModuleFullySelected(subPermissions);
  };
  ...
  const handleCreateUser = async (e) => {
    ...
    if (!userFormData.rol_grupo || !userFormData.rol_grupo.trim()) {
      setError('Debe seleccionar un cargo para el usuario');
      return;
    }
    ...
    try {
      ...
      console.log('📤 Enviando datos para crear usuario:', {
        ...
        password: '***',
        rol_grupo: userFormData.rol_grupo,
        permisos_especiales: userFormData.permisos_especiales
      });
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex items-center space-x-1">
                          ...
                          {renderSortIcon('departamento_nombre')}
                        </div>
                      </th>
                      <th 
                        className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-teal-50 transition-colors"
                      ...
                      >
                        ...
                        <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                          <div>
                            ...
                            {/* Mostrar cargo y departamento en móviles */}
                            <div className="sm:hidden mt-2">
                              ...
                              {trabajador.departamento_nombre && (
                                <div className="text-xs text-green-600 bg-green-50 px-2 py-1 rounded inline-block mr-1 mb-1">
                                  {trabajador.departamento_nombre}
                                </div>
                              )}
                              {trabajador.rol_nombre && (
                        ...
                        <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden sm:table-cell">
                          {trabajador.cargo || <span className="text-gray-400 italic">Sin cargo asignado</span>}
                        </td>
      ...
      {showUserModal && selectedWorker && (
        <div className={`fixed inset-0 flex items-center justify-center z-50 p-4 ${styles.modalOverlay}
          ...
          <div className={`bg-white rounded-lg shadow-xl w-full max-w-lg h-[80vh] max-h-[600px] ${styles.modalContent}
              ...
              <form id="user-form" onSubmit={handleCreateUser} className="space-y-4">
                  ...
                  >
                    {availableRoles.map((rol) => (
                      <option key={rol.id} value={rol.nombre}>
                        {rol.nombre.charAt(0).toUpperCase() + rol.nombre.slice(1).replace(/_/g, ' ')}
                      </option>
                    ))}
                  </select>
                  <p className="text-xs text-gray-500 mt-1">
                    Selecciona el cargo que mejor describa su posición en la empresa
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto border border-gray-200 rounded-md p-3">
                    {/* Gestión de Trabajadores */}
                    {(() => {
                      const trabajadoresPermisos = [
                        { codigo: 'trabajadores_leer', nombre: 'Ver trabajadores' },
                        { codigo: 'trabajadores_escribir', nombre: 'Agregar trabajadores' },
                        { codigo: 'trabajadores_editar', nombre: 'Editar trabajadores' },
                        { codigo: 'trabajadores_eliminar', nombre: 'Eliminar trabajadores' }
                      ];
                      const isExpanded = expandedModules.has('trabajadores');
                      const isFullySelected = isModuleFullySelected(trabajadoresPermisos);
                      const isPartiallySelected = isModulePartiallySelected(trabajadoresPermisos);
                      
                      return (
                        <div className="border border-gray-200 rounded-md">
                          <div className="flex items-center p-2 bg-gray-50 hover:bg-gray-100 transition-colors">
                            <input
                              type="checkbox"
                              ...
                              onChange={() => toggleModuleSelection('trabajadores', trabajadoresPermisos)}
                              className="h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded mr-2"
                            />
                            <button
                              type="button"
                              onClick={() => toggleModuleExpansion('trabajadores')}
                          ...
                          {isExpanded && (
                            <div className="p-2 space-y-2 bg-white border-t border-gray-200">
                              {trabajadoresPermisos.map((permiso) => (
                                <label key={permiso.codigo} className="flex items-center space-x-2 ml-4">
                                  <input
                                    type="checkbox"
                                  ...
                                  />
                                  <span className="text-xs text-gray-600">{permiso.nombre}</span>
                                </label>
                              ))}
                            </div>
                          )}
                    ...
                    {(() => {
                      const inventarioPermisos = [
                        ...
                        { codigo: 'inventario_escribir', nombre: 'Agregar inventario' },
                        { codigo: 'inventario_editar', nombre: 'Editar inventario' },
                        { codigo: 'inventario_eliminar', nombre: 'Eliminar del inventario' }
                      ];
                      const isExpanded = expandedModules.has('inventario');
                      const isFullySelected = isModuleFullySelected(inventarioPermisos);
                      const isPartiallySelected = isModulePartiallySelected(inventarioPermisos);
                        ...
                        <div className="border border-gray-200 rounded-md">
                          <div className="flex items-center p-2 bg-gray-50 hover:bg-gray-100 transition-colors">
                            <input
                              ...
                              onChange={() => toggleModuleSelection('inventario', inventarioPermisos)}
                              className="h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded mr-2"
                            />
                            <button
                              type="button"
                              onClick={() => toggleModuleExpansion('inventario')}
                              className="flex items-center justify-between w-full text-left"
                            >
                              <span className="text-sm font-medium text-gray-700">
                                📦 Gestión de Inventario
                          ...
                          {isExpanded && (
                            <div className="p-2 space-y-2 bg-white border-t border-gray-200">
                              {inventarioPermisos.map((permiso) => (
                                <label key={permiso.codigo} className="flex items-center space-x-2 ml-4">
                                  <input
                                    type="checkbox"
                                    checked={userFormData.permisos_especiales.includes(permiso.codigo)}
                    ...
                    {/* Gestión de Copias */}
                    {(() => {
                      const copiasPermisos = [
                        { codigo: 'copias_leer', nombre: 'Ver copias' },
                        { codigo: 'copias_escribir', nombre: 'Registrar copias' },
                        { codigo: 'copias_editar', nombre: 'Editar copias' },
                        { codigo: 'copias_eliminar', nombre: 'Eliminar copias' }
                      ];
                      const isExpanded = expandedModules.has('copias');
                      const isFullySelected = isModuleFullySelected(copiasPermisos);
                      const isPartiallySelected = isModulePartiallySelected(copiasPermisos);
                        ...
                        <div className="border border-gray-200 rounded-md">
                          <div className="flex items-center p-2 bg-gray-50 hover:bg-gray-100 transition-colors">
                            <input
                              ...
                              onChange={() => toggleModuleSelection('copias', copiasPermisos)}
                              className="h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded mr-2"
                            />
                            <button
                              type="button"
                              onClick={() => toggleModuleExpansion('copias')}
                              className="flex items-center justify-between w-full text-left"
                            >
                              <span className="text-sm font-medium text-gray-700">
                                📄 Gestión de Copias
                    ...
                    {(() => {
                      ...
                      const isExpanded = expandedModules.has('otros');
                      const isFullySelected = isModuleFullySelected(otrosPermisos);
                      const isPartiallySelected = isModulePartiallySelected(otrosPermisos);
                        ...
                        <div className="border border-gray-200 rounded-md">
                          <div className="flex items-center p-2 bg-gray-50 hover:bg-gray-100 transition-colors">
                            <input
                              ...
                              onChange={() => toggleModuleSelection('otros', otrosPermisos)}
                              className="h-4 w-4 text-teal-600 focus:ring-teal-500 border-gray-300 rounded mr-2"
                            />
                            <button
                              type="button"
                              onClick={() => toggleModuleExpansion('otros')}
                              className="flex items-center justify-between w-full text-left"
                            >
                              <span className="text-sm font-medium text-gray-700">
                                ⚙️ Otros Permisos
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Haz clic en los módulos para expandir sus permisos específicos
                  </p>
                </div>
              </form>
            </div>
              ...
              {externalManageType === 'departamento' ? (
                departamentos.length === 0 ? (
                  ...
                  departamentos.map((dept, index) => (
                    <div key={dept.id} className="flex items-center justify-between bg-gray-50 px-3 py-2 rounded-lg hover:bg-gray-100 transition-colors duration-200">
                ...
                ocupaciones.length === 0 ? (
                  ...
                  ocupaciones.map((ocupacion, index) => (
                    <div key={ocupacion} className={`flex items-center justify-between px-3 py-2 rounded-lg transition-colors duration-200 ${
